...
Start Docker Desktop/daemon, then run this script again:
  ./scripts/start-no-root-avd.sh
MSG
    exit 1
  fi
}

export COMPOSE_PROJECT_NAME="${COMPOSE_PROJECT_NAME:-dockerify-android-noroot}"
export ANDROID_CONTAINER_NAME="${ANDROID_CONTAINER_NAME:-dockerify-android-noroot}"
export SCRCPY_CONTAINER_NAME="${SCRCPY_CONTAINER_NAME:-scrcpy-web-noroot}"
export DATA_DIR="${DATA_DIR:-./data/no-root}"
